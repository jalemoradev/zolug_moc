...
- **Derecha — buscador:** un control compuesto = **selector** (`Nombre` / `Cédula` / `Teléfono`) + **input** de búsqueda con icono de lupa. El selector define por cuál campo se busca.
...
### 3. Paginador
- **Izquierda:** rango **"Mostrando a–b de N"** + **filas por página** (selector 8 / 25 / 50).
...
- **Búsqueda en vivo:** filtra por el campo del selector; para **Cédula/Teléfono** ignora puntos y espacios (compara solo dígitos); al escribir vuelve a la **página 1**; si nada coincide, muestra el estado **"Sin resultados"**.
...
## Accesibilidad
`:focus-visible` global; filas operables con teclado (`Enter`); `aria-label` en filas y controles de paginación; buscador con `aria-label` en selector e input. Escala tipográfica de `base.html` (piso 14 / cuerpo 16), tabular-nums en documento/teléfono. Color al mínimo.
